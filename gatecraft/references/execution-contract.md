...
- **Action:** Check and resolve the optional OmniRoute installation/use policy under `omniroute.md` at the start of every orchestration session by invoking `scripts/omniroute-session.ps1 status` and `resolve-policy -RepositoryRoot <target-repo>`, observing actual endpoint/adapter state rather than a persisted installation boolean and keeping install consent separate from use consent; treat a confirmed install as complete only when the package postinstall has run and `install-health` passes, never on npm's exit code alone, and never register providers, create accounts, or handle keys on the user's behalf; preflight a selected source mode before launch, never silently switch modes or build without a separate hash-bound attended confirmation, and require managed source startup to prove loopback binding plus bounded sanitized diagnostics; then enumerate every configured vendor profile, assign canonical slugs, identify the current orchestrator, load and classify the local `model-catalog/v1` record under `model-catalog.md` (including 72-hour freshness and startup-only refresh authority), record exact direct or selected-gateway launch/PID/process-group/start/reap manifests, and smoke-test self-identification, usage (including separate short-session and weekly window availability), non-interactive launch, ACK, process-tree reap, and write capabilities for every candidate seat without requiring every usage window to be available.
- **Evidence:** Record the sanitized OmniRoute decision source/state/adapter or skip reason, vendor counts, labels, slugs, manual-dispatch-only classifications, role choices, per-window usage capability results, and exact tested invocations without gateway credentials, raw catalogs, prompts, or provider responses.
- **Stop:** Stop gateway activation on missing install consent, invalid preferences, broken catalog, unapproved remote/autostart behavior, readiness timeout, secret-bearing evidence, unknown effective routing for a required explicit review, or an unbounded compression boundary; because the gateway is optional, continue with direct profiles only under the documented attended decision or unattended fallback. Stop automatic dispatch or succession for any selected profile that cannot launch, identify itself, ACK, or reap reliably; classify it by capability instead of inventing another mode.
...
- **Action:** Persist either never-push or dedicated-non-main-branch auto-push, keep main pushes and every deploy ask-before unless explicitly authorized, and separate remote policy from local verified merge authority. Persist the chosen mode in `.beads/gatecraft-push-policy.json` (`gatecraft-push-policy/v1`) so `scripts/enforce-gate.ps1 check-push` can validate it mechanically. When `scripts/install-githooks.ps1`'s real `pre-push` hook (gatecraft-7dq) is installed, export `GATECRAFT_AUTOMATED_PUSH=1` only for a push this policy is meant to gate — never for a push the user directly typed or explicitly approved.
- **Evidence:** Record the chosen push target, deploy rule, user authorization, persistence key, and (when installed) the git-hook enforcement state.
...
- **Trigger:** Begin every session — not only the first bootstrap ever performed on a machine/project — by checking for external merge drift before claiming or dispatching any bead, and periodically recheck while selecting work.
- **Action:** Check for drift by first running an explicit `git fetch` against the configured remote and comparing `git rev-list --count HEAD..origin/main` and `git rev-list --count origin/main..HEAD` in both directions; run the shared-state equivalent (e.g. `bd dolt pull`) and observe its result. Do not infer this comparison from local `git log`/`git status` alone or assume prior sessions already synced — the check is only satisfied by an explicit fetch/pull executed in this session. If origin/the shared remote is ahead by any amount, stop and show the user the missing commits/records before starting any new dispatch or work — do not treat "nothing to compare" as equivalent to "already checked." Only after confirming no drift (or after the user has decided how to reconcile it), compare merges and closed beads against exact VERIFIED ... result=pass evidence, classify unmatched work as drift, and ask the user to choose catch-up scope when drift is material. Only in attended mode and after a direct answer, optionally emit one validated `gatecraft-recovery/v1` audit record binding the exact external merge OID, bead ID or stable drift ID, current artifact SHA, explicit observation time, sanitized missing-evidence reason, and sanitized direct-user decision; keep it separate from verification/v2. Keep the raw line and detailed validation result local, and derive durable/shared evidence only with `ConvertTo-GatecraftRecoveryProjection`.
- **Evidence:** Record the exact fetch/pull commands run this session, their observed ahead/behind counts and result, compared ranges, matching pass ledgers, unmatched counts and kinds, and the user's prioritization decision. When recovery is emitted, persist only the durable-safe projection containing the recovery ID, external merge OID, bead/drift subject ID, current artifact SHA, observation time, omitted-field markers, and `audit-only` disposition; the missing-evidence and direct-user free text remains local by default.
- **Stop:** Stop before any claim, dispatch, or new work in this session when the explicit fetch/pull step has not been run, or when it shows origin/the shared remote ahead and the user has not yet been shown the gap and decided how to proceed. Stop treating closure, VERIFICATION_FAILED, missing ledger lines, or any recovery record as proof of verification. Never emit recovery in unattended mode; never count it as integration/premerge, postmerge `VERIFIED result=pass`, `REVIEW_PASS`, a replacement phase, or an ordered verification/v2 chain repair regardless of position or SHA equality.

  **Lived incident (2026-07-24, OrizzonteDiploma):** a session interpreted this step as "compare local merges against known VERIFIED beads" only, without ever running `git fetch`. Origin/main was 52 commits ahead from a separate same-account orchestration session on another machine, undetected all day. Consequence: 6-7 already-completed beads were independently redone, including a structurally incompatible refactor of the same file that would have produced a PHP fatal error on merge, plus a colliding bead ID silently allocated for a different origin record. `bd dolt pull` correctly aborted the automatic merge (clean abort, no corruption) but the git-side drift was only discovered when the user asked for an end-of-day push. This is why the explicit fetch/pull is a per-session action, not a one-time-ever bootstrap check.
...
- **Action:** Set BEADS_ACTOR, bind the launch to the reserved attempt and valid stable worker ID, apply the session's direct-or-OmniRoute launch manifest without writing ordinary CLI configuration, redirect output to a unique raw attempt log, close stdin as required, verify startup, exact process identity, and any required effective model/thinking identity, increment total-spawn state only after accepting that identity-bound spawn, monitor only new log tails and size/mtime, and reap the recorded process tree on a confirmed stall.
- **Evidence:** Record the sanitized gateway mode and compression boundary, launch command without credentials, actor, stable worker ID, effective model/thinking when required, PID/process group, reserved attempt ID, total-spawn count, attempt-log path and access check, startup observation, tail offsets, process/workspace state, and termination result.
...
- **Action:** Verify main cleanliness, integrate moved main, re-gate the exact candidate, emit one integration/premerge receipt, obtain the required SHA-bound Step 2.5 review and re-review requested fixes, run the read-only dispatch-baseline foreign-change sweep immediately before commit/merge, record the pre-merge SHA, invoke `scripts/enforce-gate.ps1 check-merge --repository-root <root> --bead-id <id> --artifact-sha <candidate-SHA>` (its argument-preserving POSIX `scripts/enforce-gate.sh` wrapper on non-Windows; review is required by default, so pass `--low-risk-no-review-required` only when GC-1.7's risk tier and GC-0.10 genuinely did not require a review) as a blocking precondition immediately before the local merge itself; when `scripts/install-githooks.ps1`'s real `pre-merge-commit` hook (gatecraft-7dq) is installed, also export `GATECRAFT_BEAD_ID` (and `GATECRAFT_ARTIFACT_SHA`/`GATECRAFT_LOW_RISK_NO_REVIEW_REQUIRED` as applicable) in the environment used for the `git merge` command itself, so git invokes the same check automatically even if the explicit call above is skipped; merge locally only on its exit 0, re-gate main, emit the postmerge receipt, apply only the constrained reversible-code rollback, stop/reap workers before cleanup, and only after binding the exact expected merged SHA/status create a second create-only postmerge foreign baseline under a fresh ID for the pre-cycle-end sweep with only the processes still expected live.
- **Evidence:** Record integration receipt ID and artifact SHA, reviewer identity, source/review identity, review receipt sequence and verdict, sanitized pre-merge sweep marker/reason, `enforce-gate.ps1 check-merge` exit code and, on failure, its exact `code=` token, pre-merge and merged SHAs, conflict-resolution verification, postmerge receipt, rollback eligibility, cleanup result, fresh postmerge baseline ID, and its sanitized success marker without raw status/process data.
- **Stop:** Stop merge on a malformed, blocked, inconclusive, swapped, repeated-clarification, or artifact-mismatched review; dirty or moved unverified main; nonzero foreign sweep; failed combined gate; any nonzero `enforce-gate.ps1 check-merge` exit (this is a blocking precondition independent of the orchestrator's own memory of GC-0.0/GC-1.4/GC-1.7 — a missing local guard, missing baseline receipt, or missing SHA-bound review fails the merge mechanically, not by prose discipline); irreversible side effects; ambiguous rollback; or any post-merge gate failure that cannot be safely reverted. Stop cycle-end if the fresh postmerge baseline cannot be created only after exact postmerge state is verified. A sweep finding is observation only and never authorizes staging, absorbing, or reverting the foreign path.

### GC-1.11 — Verification ledger and close

- **Trigger:** Record bead state after independent verification and the main gate complete.
- **Action:** Validate the ordered verification/v2 chain with Gatecraft.Protocol.psm1, invoke `scripts/enforce-gate.ps1 check-close --repository-root <root> --bead-id <id>` (or its `.sh` wrapper) as a blocking precondition and proceed only on its exit 0 (it reuses this same `Test-GatecraftVerificationChain` validation rather than a second implementation, so a chain gap fails mechanically before `bd close` runs, not only by the orchestrator remembering to check), lead a passing comment with the backward-compatible VERIFIED ... result=pass postmerge line or a failing comment with VERIFICATION_FAILED ... result=fail, sanitize the receipt-derived projection, close only after Decision=pass, and reopen failures under the retry policy.
- **Evidence:** Record the exact final ledger line, baseline/integration/review references, machine reason codes, `enforce-gate.ps1 check-close` exit code and, on failure, its exact `code=` token, sanitization check, comment identifier, bead status, close or reopen result, and refreshed lock heartbeat.
- **Stop:** Stop closure when the baseline observation is missing, malformed, conflicting, mislabelled, non-unsigned, misordered, or unreferenced; a nonzero baseline lacks `baseline-expected-gap` in either declared or observed evidence; integration is missing, malformed, nonzero, or non-pass; review is inadmissible or SHA-mismatched; final exit is nonzero; references are broken; no exact final pass ledger exists; a `RECOVERY` record is supplied as qualification or chain repair; `enforce-gate.ps1 check-close` exits nonzero; or closure triggers irreversible effects before verification.

### GC-1.12 — Cycle-end persistence

- **Trigger:** Finish every bead cycle immediately after ledger and status updates.
- **Action:** Append exactly one `cycle-end` event only after the read-only sweep of GC-1.10's fresh postmerge foreign baseline immediately before cycle-end exits zero, through `scripts/cycle-end.ps1` or its argument-preserving POSIX `scripts/cycle-end.sh` wrapper under a caller-supplied local state root, with a stable event ID and the next strictly monotonic positive sequence; persist the append-only canonical receipt first and derive the session-log, heartbeat, snapshot, and dashboard projections only from the validated receipt sequence; replay an identical event to repair interruption. Before the next claim, invoke at most one applicable usage adapter and persist its sanitized `USAGE_CHECK` record with the completed cycle ID, adapter, `ok`/`no-data`/`skip` state, short-session and weekly availability, reason code, and UTC check time; enforce raw-log retention and restrictive access; update reliability history; push or deploy only under GC-0.9 and explicit authority; and release the cooperative local guard only by its exact owner after a terminal/local boundary when no more local orchestration mutation is pending.
- **Evidence:** Record the sanitized pre-cycle-end sweep marker/reason, event ID, cycle sequence, normalized timestamp, mode, sanitized summary check, canonical receipt path, `projections=complete` exit-0 marker or visible nonzero failure, one machine-greppable `USAGE_CHECK` line, retention disposition, reliability update, terminal condition, exact-owner release marker when applicable, and authorized remote action or local-only outcome.
- **Stop:** Stop before cycle-end on any nonzero foreign sweep, and stop the next bead claim until the exact event exits zero with every projection complete and its exactly-one sanitized `USAGE_CHECK` record is durable; reject same-ID payload conflicts, sequence reuse, gaps, malformed/escaping paths, unknown input, duplicate same-cycle usage checks, or a missing check. On projection failure, stop nonzero: unattended mode fails closed, while attended mode may expose only the documented `automatic_completion=false` manual checklist and must not claim automatic completion. Never release another owner's guard, release before the terminal/local boundary, mutate a sweep finding, or publish raw logs/status/process evidence.

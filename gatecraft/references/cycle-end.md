...
`cycle-end` is the only *durable, sequenced* event in the MVP. Invoke it once at GC-1.12 after the verification ledger and bead status are current and after the separate `local-guard.md` sweep of GC-1.10's fresh verified-postmerge baseline exits zero. The caller supplies sanitized, stable values; the command does not read a raw worker log, tracker, repository, clock, or network. The sweep is an explicit preceding call, not a new cycle event or ledger input.

Alongside it there is exactly one much lighter-weight marker, not a receipt: the **cycle-begin marker**. `New-GatecraftCycleBeginMarker` (in `scripts/Gatecraft.Protocol.psm1`) is an orchestrator-level primitive called at GC-1.10, immediately before merge begins — it is never invoked by `cycle-end.ps1` itself. It create-only writes `cycle-end/in-progress.marker` under the same state root, recording the target cycle sequence (the latest completed receipt's sequence plus one) and a canonical UTC timestamp, so that `Test-GatecraftReclaimBoundary` can tell a merge/ledger/cycle-end sequence is in flight even though the previous receipt and dashboard still agree with each other throughout GC-1.10–GC-1.12. `cycle-end.ps1` deletes this marker as the very last action of a successful run, strictly after `projections=complete` is achieved, and only when the marker's recorded target sequence exactly equals the sequence that specific invocation just completed — a crash between projection completion and marker deletion conservatively still reads as in-progress, and an identical replay reaches and retries the deletion. A marker present but targeting a different sequence (for example, a delayed replay of an older, already-completed cycle-end racing a newer cycle's live in-flight marker) is left untouched rather than deleted, and a malformed marker is likewise left untouched; neither case is treated as a failure of that invocation's own completed work. `Test-GatecraftReclaimBoundary` in turn treats the marker as a pure existence check — its target sequence is never compared — so a marker present for any reason, including the sequence just completed, blocks reclaim until it is actually cleared.

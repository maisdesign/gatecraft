...
**The real reason a dashboard looks stale is usually one layer deeper than the fetch/snapshot split above: `.beads/issues.jsonl` itself is a manual, point-in-time export, not a live mirror of `bd`'s Dolt/SQLite DB.** `bd close`/`update`/`defer` and friends never touch `issues.jsonl` — only `bd export` does, and nothing calls it automatically (`bd`'s own `hooks` subsystem wraps *git* lifecycle events — pre-commit/post-merge/pre-push/post-checkout — not bd's internal writes, and there is no `bd export --watch`/daemon mode). So even in the "live fetch, polls every 15s" HTTP-served case above, the panel can be polling a file that hasn't changed in hours: a bead deferred or closed minutes ago still shows as `ready`/open until someone re-exports. The correct refresh sequence, every time, in this order:

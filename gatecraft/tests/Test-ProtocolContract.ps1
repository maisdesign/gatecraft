...
$debateGuiIntegrationPath = Join-Path $repoRoot 'gatecraft/references/debategui-integration.md'
...
$debateGuiIntegration = Read-RequiredText -Path $debateGuiIntegrationPath -Label 'DebateGUI integration ADR'
...
Assert-Match -Text $localGuardReference -Pattern 'complete raw bytes and SHA-256 of `git status --porcelain=v1 -z --untracked-files=all --ignore-submodules=none`' -Message 'Local guard reference must define the complete raw status baseline without honoring submodule-ignore config.'
...
Assert-Match -Text $guardScript -Pattern "@\('status', '--porcelain=v1', '-z', '--untracked-files=all', '--ignore-submodules=none'\)" -Message 'Guard must capture complete porcelain state without honoring submodule-ignore config.'
...
Assert-Match -Text $debateGuiIntegration -Pattern 'gatecraft-debategui/v1' -Message 'DebateGUI ADR must define a versioned protocol.'
Assert-Match -Text $debateGuiIntegration -Pattern 'explicit interactive `install-debategui` action' -Message 'DebateGUI acquisition must remain opt-in.'
Assert-Match -Text $debateGuiIntegration -Pattern 'SHA-256 before extraction' -Message 'DebateGUI installer must verify the pinned archive.'
Assert-Match -Text $debateGuiIntegration -Pattern 'instances\.json' -Message 'DebateGUI ADR must define a local instance registry.'
Assert-Match -Text $debateGuiIntegration -Pattern 'binds `127\.0\.0\.1` only' -Message 'DebateGUI bridge must remain loopback-only.'
Assert-Match -Text $debateGuiIntegration -Pattern 'never contain absolute paths' -Message 'DebateGUI descriptor privacy boundary must reject paths.'
Assert-Match -Text $debateGuiIntegration -Pattern 'One installed DebateGUI process exposes one loopback-only multiplexed bridge' -Message 'DebateGUI ADR must select multiplexed multi-instance architecture.'
